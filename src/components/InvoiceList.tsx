  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Past Documents</h2>
          <p className="text-slate-500 text-sm">Manage entries for invoices, bills, and estimates</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        <div className="border-b border-slate-200 flex flex-nowrap overflow-x-auto hide-scrollbar">
          <button onClick={() => setFilterType('invoice')} className={`px-6 py-4 text-sm font-bold whitespace-nowrap transition-colors border-b-2 ${filterType === 'invoice' ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}>Invoices</button>
          <button onClick={() => setFilterType('estimate')} className={`px-6 py-4 text-sm font-bold whitespace-nowrap transition-colors border-b-2 ${filterType === 'estimate' ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}>Estimates</button>
          <button onClick={() => setFilterType('purchase')} className={`px-6 py-4 text-sm font-bold whitespace-nowrap transition-colors border-b-2 ${filterType === 'purchase' ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}>Purchase Bills</button>
          <button onClick={() => setFilterType('credit-debit')} className={`px-6 py-4 text-sm font-bold whitespace-nowrap transition-colors border-b-2 ${filterType === 'credit-debit' ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-slate-600 hover:bg-slate-50'}`}>Credit/Debit Notes</button>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Document Info</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Party</th>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-20 text-center text-slate-400">
                    <div className="flex flex-col items-center max-w-xs mx-auto">
                      ...
                      <p className="font-bold text-slate-500 text-lg mb-2">No Documents Found</p>
                      <p className="text-sm">Start by creating your first document.</p>
                      <button 
                        onClick={onNewInvoice}
